...
#  This script should be executed from unitigging/asm.ovlStore.BUILDING/, but the binary needs
...
if [ -e ./asm.ovlStore.BUILDING/$jobname.info -a ! -e ./asm.ovlStore.BUILDING/$jobname.started ] ; then
  echo "Sorting job finished; info file './asm.ovlStore.BUILDING/$jobname.info' exists."
  exit
fi
#
#  Sort!
...
$bin/ovStoreSorter \
  -deletelate \
  -O  ./asm.ovlStore.BUILDING \
  -S ../asm.seqStore \
  -C  ./asm.ovlStore.config \
  -f \
  -s $jobid \
  -M 2 
